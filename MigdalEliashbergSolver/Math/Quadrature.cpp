...
void generate_composite_rule(double xtab_old[], double weight_old[], size_t order_old,
    ...
    xtab_new_vec.clear();
    weight_new_vec.clear();
    
    for (size_t n=0; n<N_rep; n++) {
        
        double x_beg = x0_new + n*(x1_new-x0_new)/N_rep;
        double x_end = x0_new + (n+1)*(x1_new-x0_new)/N_rep;
        double fact = (x_end-x_beg)/(x1_old-x0_old);
        
        for (size_t i=0; i<order_old; i++) {
            
            double new_x = x_beg + (xtab_old[i]-x0_old)*fact;
            double new_w = weight_old[i]*fact;
            
            xtab_new_vec.push_back(new_x);
            weight_new_vec.push_back(new_w);
